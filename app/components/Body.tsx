...
import { ArrowLeftIcon, ChevronDownIcon, EllipsisHorizontalIcon } from '@heroicons/react/24/solid'
	...
	const sortIngredients = (m: string[]) => {
		for (let index = 1; index <= 20; index++) {
			...
			if (m['strIngredient' + index]) {
				ingredients.push(m['strIngredient' + index] + ":" + m['strMeasure' + index]);
			} else {
				break
			}
		}
			...
			{meal && (
				<div className='pt-6 relative flex flex-col h-screen items-center gap-5'>
					<div className='flex flex-row items-center justify-evenly w-full'>
						<ArrowLeftIcon className='h-8 w-8 opacity-30 hover:opacity-100 duration-300 cursor-pointer' onClick={resetMeal}/>
				...
				src={"https://www.youtube.com/embed/" + meal.strYoutube.slice(-11)}>
					...
					<div className='flex flex-row items-center justify-evenly w-full'>
						<h1 className='sm:text-2xl text-xl font-bold text-indigo-900'>Ingredients ({ingredients.length})</h1>
					...
					<div className='overflow-hidden h-full w-2/3 flex items-center'>
